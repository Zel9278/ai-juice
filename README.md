...
``` json
{
	...
	"openaiApiKey": "OpenAI互換APIのキー(&openai, &chatgpt, &chatgpt4 のいずれかのメンションで呼び出せる)。OpenAI以外にも、Chat Completions APIと互換性のあるエンドポイント(ローカルLLM含む)ならこれで利用可能",
	"openaiApiUrl": "OpenAI互換APIのエンドポイントURL(オプション、デフォルトはOpenAI公式の https://api.openai.com/v1/chat/completions )",
	"openaiModel": "OpenAI互換APIで使うモデル名(オプション、デフォルトは gpt-4o-mini)",
	"openaiReasoningEffort": "OpenAI互換APIに送るreasoning_effort(オプション、例: low / medium / high。指定できる値はモデルによる。未指定なら送らない)",
